...
def _master_bound(inst: Instance, master: AssignmentMaster) -> float | None:
    ...
    instance; on later solves it bounds every not-yet-evaluated assignment.
    Capacity-screened and daycap-penalized solves are rejected above."""
    if (master.last_status != "OPTIMAL" or master.last_z2 is None
            or master.last_capacity_active):
        return None
